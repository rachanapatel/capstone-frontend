...
const NonManagerPastShifts = ({ user }) => {
...
useEffect(() => {
    const fetchShifts = async () => {
      try {
        ...
        console.log('Raw Shifts:', shiftRes.data);
        
        const today = new Date();
        const pastShifts = shiftRes.data
          
          .filter((s) => {
            const isPast = new Date(s.starttime) < today;
            console.log('Shift:', s, 'Is Past:', isPast); 
            return isPast;
          })
      
          .filter((s) => {
            const isEmployeeMatch = s.employee === user.id;
            console.log('Shift:', s, 'Employee Match:', isEmployeeMatch); 
            return isEmployeeMatch;
          });
        
        console.log('Filtered Past Shifts:', pastShifts);
        setShifts(pastShifts);
      } catch (error) {
        console.error("Failed to load shifts:", error);
      } finally {
...
export default NonManagerPastShifts;